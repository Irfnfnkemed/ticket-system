...
public:
    ...
    void add_user(char user_name_[], char password_[], char name_[],
                  char mail_addr_[], char cur_user_name_[] = nullptr,
                  int privilege_ = 10) {
        if (Account.empty()) {
            Account.add_user(user_name_, password_, name_, mail_addr_, privilege_);
        } else {
    ...
    void query_profile(char cur_user_name_[], char user_name_[]) {
        ...
        if (cur_privilege == -1) { printf("-1\n"); }//未登录
            ...
            Account.set_user(user_name_);//设置要查询的账户
    ...
    void modify_profile(char cur_user_name_[], char user_name_[], bool flag[],
        ...
        int cur_privilege = log_account.find(User_name(cur_user_name_));
        if (cur_privilege == -1) { printf("-1\n"); }//未登录
        else if (privilege_ >= cur_privilege) { printf("-1\n"); }//权限错误
        else {
            Account.set_user(user_name_);
            ...
            if (Account.modify_user(user_name_) && flag[3]) {//检查已登录账户的权限
